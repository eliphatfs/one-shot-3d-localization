...
def real2prob(val, max_val, num_bins, circular=False):
    ...
    if not circular:
        ...
        if is_torch:
            ...
            low = np.clip(np.floor(val / interval).astype(np.int), a_min=None, a_max=num_bins - 2)
        high = low + 1
...
def process_data(mask_im, coord_map, inst_dict, meta_path):
    ...
    for i, line in enumerate(lines):
        ...
        if len(words) == 3:
            ## real scanned objs
            if words[2][-3:] == 'npz':
                npz_path = os.path.join('NOCS/obj_models/real_val', words[2])
                with np.load(npz_path) as npz_file:
                    scale_factor[i, :] = npz_file['scale']
            else:
                bbox_file = os.path.join('NOCS/obj_models', 'real_test', words[2]+'.txt')
